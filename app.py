...
# Fungsi untuk mendapatkan chat ID dari bot
def get_chat_id():
...
with st.sidebar:
    ...
    # Coba dapatkan chat ID secara otomatis
    chat_id = get_chat_id()
    
    if chat_id:
        st.success(f"Chat ID ditemukan: {chat_id}")
        telegram_chat_id = st.text_input("Chat ID Telegram", value=str(chat_id))
    else:
        st.warning("Kirim pesan ke bot untuk mendapatkan Chat ID")
        telegram_chat_id = st.text_input("Chat ID Telegram", placeholder="Masukkan Chat ID")
    ...
    **Cara mendapatkan Chat ID:**
    ...
    3. Bot akan membalas dengan informasi ID Anda
    """)
...
with st.form("survey_form"):
    ...
    if submitted:
        ...
        if not telegram_chat_id: error_messages.append("Chat ID Telegram harus diisi")
...
<b>Hambatan:</b> {obstacles if obstacles else 'Tidak ada'}
...
<b>Waktu Survey:</b> {survey_data['timestamp']}
                    ...
                    if message_result and message_result.get('ok'):
                        ...
                        st.error("Gagal mengirim data ke Telegram. Periksa Chat ID dan koneksi internet.")
